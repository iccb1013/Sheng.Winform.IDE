...
{
    partial class ExplorerView : WorkbenchViewBase
    {
        public const string SINGLEKEY = "DataEntityExplorerView";
        ...
        public ExplorerView()
        {
            ...
            this.Icon = DrawingTool.ImageToIcon(Resources.DataEntity);
            this.TabText = Language.Current.DataEntityView_TabText;
            if (DesignMode)
                return;
        ...
        private void FormDataEntity_Load(object sender, EventArgs e)
        {
        ...
        {
            DataBind(e.DataBoundType, e.DataBoundItem, e.Items, e.ItemType);
        }
        void _gridView_GridDoubleClick(object sender, ExplorerGridContainer.GridDoubleClickEventArgs e)
        {
            if (e.Data == null)
                return;
            if (e.Data.GetType().Equals(typeof(DataEntityDev)))
            {
        ...
        private void DataBind(Type dataType, object data, IList items, Type itemType)
        {
            if (data == null)
            {
                _gridView.Clear();
            ...
            {
                _gridView.DataBind(dataType, items, itemType, data);
            }
        }
