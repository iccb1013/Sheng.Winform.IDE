...
namespace Sheng.SailingEase.Controls.Docking
{
	...
	public partial class DockPane : UserControl, IDockDragSource
	{
        ...
        internal void ShowTabPageContextMenu(Control control, Point position)
        {
            ...
            {
                //cxs
                //如果选项卡显示在底部，那么快捷菜单就在上方显示
                //contextMenuStrip.Show(control, position);

                if (this.DockPanel.DocumentTabStripLocation == DocumentTabStripLocation.Top)
                    contextMenuStrip.Show(control, position);
                else
                    contextMenuStrip.Show(control, position, ToolStripDropDownDirection.AboveRight);
