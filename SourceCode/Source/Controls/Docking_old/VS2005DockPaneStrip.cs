...
namespace Sheng.SIMBE.SEControl.Docking
{
	internal class VS2005DockPaneStrip : DockPaneStripBase
	{
        ...
        //cxs
        //选项卡最少保证显示的宽度
        private const int _DocumentTabMinWidth = 80;
        ...
        {
            //cxs
            //选中的选项卡的背景，不用这个了，直接在绘制背景的地方new brush
            //因为这个属性是静态的，不好取Rectangle
            get { return SystemBrushes.ControlLightLight; }
        }
        ...
        private static Brush BrushDocumentInactiveBackground
        {
            ...
            get { return new LinearGradientBrush(new Point(0, 0), new Point(0, 10), Color.Red, Color.Yellow); }
        }
		...
		public VS2005DockPaneStrip(DockPane pane) : base(pane)
		{
            ...
            //cxs,为窗体选择菜单附加样式
            SEToolStripRender toolStripRender = new SEToolStripRender();
            toolStripRender.Panels.ContentPanelTop = Color.FromArgb(241, 239, 228);

            toolStripRender.Toolstrip.BackgroundTop = Color.FromArgb(250, 249, 245);
            toolStripRender.Toolstrip.BackgroundBottom = Color.FromArgb(200, 199, 178);
            toolStripRender.Toolstrip.BorderBottom = Color.FromArgb(200, 199, 178);

            toolStripRender.Toolstrip.Curve = 0;
            toolStripRender.AlterColor = true;
            toolStripRender.OverrideColor = Color.Black;

            m_selectMenu.Renderer = toolStripRender;
        ...
        private GraphicsPath GetOutline_Document(int index)
        {
            ...
            //cxs
            // 根据选项卡位置的不同,使用不同的绘制方法

            //GraphicsPath pathTab = GetTabOutline_Document(Tabs[index], true, true, true);

            GraphicsPath pathTab;

            if (this.DockPane.DockPanel.TabAlign == TabAlign.Top)
                pathTab = GetTabOutline_Document(Tabs[index], true, true, true);
            else
                pathTab = GetTabOutline_Document2(Tabs[index], true, true, true);
        ...
        private bool CalculateDocumentTab(Rectangle rectTabStrip, ref int x, int index)
        {
            ...
            //cxs
            //如果宽度少于最少宽度,则设为最少宽度
            width = Math.Max(tab.MaxWidth, DocumentTabMinWidth);
		...
		private void DrawTabStrip_Document(Graphics g)
		{
            ...
            {
                //画到上面去
                g.DrawLine(PenDocumentTabActiveBorder, rectTabStrip.Left, rectTabStrip.Top,
		...
		private Rectangle GetTabRectangle_Document(int index)
		{
            ...
            //cxs
            //如果选项卡处于底部,选项卡绘制区域需要进行一些位置调整,在上面的,靠下,在下面的,靠上

            if(this.DockPane.DockPanel.TabAlign == TabAlign.Top)
			    return new Rectangle(tab.TabX, rectTabStrip.Y + DocumentTabGapTop, tab.TabWidth, rectTabStrip.Height - DocumentTabGapTop);
            else
                return new Rectangle(tab.TabX, rectTabStrip.Y - 1, tab.TabWidth, rectTabStrip.Height - DocumentTabGapTop);
		}
        ...
        private GraphicsPath GetTabOutline(Tab tab, bool rtlTransform, bool toScreen)
        {
            ...
            else
            {
                ...
                if(this.DockPane.DockPanel.TabAlign == TabAlign.Top)
                    ...
                    return GetTabOutline_Document2(tab, rtlTransform, toScreen, false);
            }
        }
        ...
        /// <param name="rtlTransform"></param>
        /// <param name="toScreen"></param>
        /// <param name="full"></param>
        /// <returns></returns>
        private GraphicsPath GetTabOutline_Document2(Tab tab, bool rtlTransform, bool toScreen, bool full)
        {
            //cxs
            //绘制选项卡处于下方时的样式
            ...
            if (tab.Content == DockPane.ActiveContent || Tabs.IndexOf(tab) == StartDisplayingTab || full)
            {
                ...
                else
                {
                    ...
                    //this.CreateGraphics().DrawLine(Pens.Cyan, rect.Left, rect.Top, rect.Left - rect.Height / 2, rect.Top);

                    //this.CreateGraphics().DrawLine(Pens.Black,
                    //    rect.Left - rect.Height / 2, rect.Top,
                    //    rect.Left + rect.Height / 2, rect.Bottom);
            ...
            else
            {
                ...
                {
                    //this.CreateGraphics().DrawLine(Pens.Blue, rect.Left, rect.Top,rect.Left, rect.Bottom - rect.Height / 2);

                    //this.CreateGraphics().DrawLine(Pens.Red, 
                    //    rect.Left, rect.Bottom - rect.Height / 2, rect.Left + rect.Height / 2 + curveSize / 2, rect.Bottom + curveSize / 2
                    //   );

                    GraphicsPath.AddLine(rect.Left, rect.Top, rect.Left, rect.Bottom - rect.Height / 2);
            ...
            {
                //顶部线条和圆弧
                //实际上到我这里已经画到下面了,下部线条和圆弧

                //直线
                //this.CreateGraphics().DrawLine(Pens.DarkGreen,
                //     rect.Left +  curveSize*2 , rect.Bottom, rect.Right - curveSize / 2, rect.Bottom);
                ////弧
                //this.CreateGraphics().DrawArc(Pens.Green,
                //    new Rectangle(rect.Right - curveSize, rect.Bottom - curveSize, curveSize, curveSize), 90, -90);

                GraphicsPath.AddLine(rect.Left + curveSize * 2, rect.Bottom, rect.Right - curveSize / 2, rect.Bottom);
                GraphicsPath.AddArc(new Rectangle(rect.Right - curveSize, rect.Bottom - curveSize, curveSize, curveSize), 90, -90);
                ...
                && !full)
            {
                ...
                else
                {
                    ...
                    //this.CreateGraphics().DrawLine(Pens.Gold,
                    //     rect.Right, rect.Top + rect.Height / 2, rect.Right, rect.Bottom - curveSize / 2);

                    //this.CreateGraphics().DrawLine(Pens.DarkOrange,
                    //      rect.Right, rect.Top + rect.Height / 2, rect.Right + rect.Height / 2, rect.Top);

                    GraphicsPath.AddLine(rect.Right, rect.Top + rect.Height / 2, rect.Right, rect.Bottom - curveSize / 2);
                    GraphicsPath.AddLine(rect.Right, rect.Top + rect.Height / 2, rect.Right + rect.Height / 2, rect.Top);
            ...
            else
            {
                ...
                {
                    //选中时右侧
                    //this.CreateGraphics().DrawLine(Pens.DarkRed,
                    //     rect.Right, rect.Bottom - curveSize / 2, rect.Right, rect.Top);
		...
		private void DrawTab_Document(Graphics g, TabVS2005 tab, Rectangle rect)
		{
            ...
            //cxs
            //如果选项卡显示在下方,文本稍向下调整
            if (this.DockPane.DockPanel.TabAlign == TabAlign.Bottom)
                rectText.Y += 3;
            ...
            if (DockPane.ActiveContent == tab.Content)
            {
                //cxs
                ////这里绘制选中的选项卡的外观
                LinearGradientBrush fillBrush = new LinearGradientBrush(TabStripRectangle, SystemColors.Window, SystemColors.InactiveCaption, LinearGradientMode.Vertical);
			...
			else
			{
                //cxs
                //这里绘制未选中的选项卡的外观
                //g.FillPath(BrushDocumentInactiveBackground, path); //这行是原来的
                //g.FillPath(new LinearGradientBrush(new Point(0, 0), new Point(0, rectTab.Height + 5), Color.White, Color.LightGray), path);
        ...
        private void WindowList_Click(object sender, EventArgs e)
        {
            ...
            foreach (TabVS2005 tab in Tabs)
            {
                ...
                item.Text = content.DockHandler.TabText;
                item.Image = content.DockHandler.Icon.ToBitmap();
                item.Tag = tab.Content;
                item.Click += new EventHandler(ContextMenuItem_Click);
                SelectMenu.Items.Add((ToolStripItem)item);
