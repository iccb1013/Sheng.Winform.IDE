...
namespace Sheng.SailingEase.Kernal
{
    ...
    /* 有关 BindingList：
     * 我定义BiindingList时，是用object作泛型参数的：BindingList<object> _bindingList
     * 如果在new 这个bindingList时，作为与之绑定的list参数就没有对象，是个空集合
     * 就会导致DataGridView里呈现不出数据，列都是空白，必须重新加载才可以
     * 如果在new的时候，本身list有数据，就不存在这个问题
     * 怀疑bindingList会通过其内部元素来作类型推倒，如果没有元素，它只能以类型参数为类型进行反射，绑定数据
     * 在此情况下，用 object 类型去拿所绑定的数据，肯定是拿不到的
     * 
     * 在复合显示多种类型的对象时（同时在列表中显示多个类型的对象）时， BindingList 的类型推倒出现一个问题
     * 因为目前对于 BindingList 的声明是在 Controller 中声明的，声明为：BindingList<object> _bindingList
     * 在呈现单一类型的数据源时（即 IList 中的对象类型是一致的），没有问题
     * 但是如果 IList 中包含多种类型的对象，则 BindgList 会使用集合中的第一个对象作类型推导
     * 然后使用推导得到的 Property 访问器调用所有后续对象的属性，那么就会出错，因为 Property 访问器与对象类型不一致
     * 考虑的解决方案：
     * 除了默认的 BindingList<object> _bindingList 声明外，增加一个用于关联  BindingList 和 Codon 的集合
     * 在添加 codon 时，可采用泛型重载，指定该 codon 要使用的 BindingList 的泛型类型
     * 然后专门为此 codon 声明一个以指明泛型参数作为类型的 BindgList，放到关联集合中
     * 在后续需要对 BindingList 进行操作时，可根据codon查找关联集合中有没有对应的BindingList
     * 如果没有，就使用默认的 BindList<object>，如果有，就使用关联的 BindingList<T>
     * 原来的 BindingList 声明要改成 IBindingList _bindingList
     * 妈的，ResetItem 方法没有声明在 IBindingList 里
     * 专门声明了一个 IBindingListEx，和 BindingListEx
     * 默认 BindingList 声明则改为 IBindingListEx _bindingList
     * 
     * 现在决定：为每个 codon 都在 bindingListPool 中分配一个 Binding 引用
     * 因为在添加 codon 时，是通过接口 ITypeBinderDataGridViewTypeCodon 来添加的
     * 而且添加复合codon时，也可以不使用类似 AddCodon<T> 这样的泛型方式，那么如果
     * 在方法内部显式判断 codon 的具体类型，会使控制器类和具体的codon实现类高度耦合
     */

    /* 在复合显示多种类型的对象时（同时在列表中显示多个类型的对象）
     * 上下文菜单：
     * 菜单应该是同一个菜单，否则在多选且同时选中不同类型的对象时，到底显示哪个对象的菜单？
     * 另外对于像“删除”这种可以应用于不同对象的同类型操作，如果放在不同的上下文菜单中，无法进行统一的操作
     * 排序：
     * 要为不同类型的对象进行排序，具体的order值可在codon中设置
     * 关于codon：
     * 除了原来的TypeCodon，还需要一个专门为复合显示多种类型对象而设置的更为复杂的codon
     * 那么需要对这两种codon提取接口，对codon的操作通过接口来进行
     * 在复合codon中的所有子codon对应的对象类型，都必须有一个共同的基类型，复合codon对应这个基类型
     * 在controller查找codon操作，以及外部调用DataBind绑定数据时，都以此基类型为当前类型
     * 如果不这样做，controller内部无法定位codon，比如在以IList进行绑定数据时，必须明确指明集合内的对象Type，
     * DataBind方法绑定过程中根据Type找对应的Codon
     * 在复合codon中包含多个普通的子codon用于表示所绑定的不同对象，
     * 后期可针对不同子对象的不同ItemType和ItemsMember属性，进一步完善功能
     * 既然所有子codon所绑定的类型都要有一个共同的基类型，为什么不直接用普通codon和actOnSubClass?
     * 因为复合codon可以对加载的IList中的对象排序，不同类型的对象可以指定不同的上下文菜单，ItemType和ItemsMember
     * 在不需要以上特性的时候，可以直接用普通codon并设置actOnSubClass=true
     */
