...
namespace ListViewTest
{
    ...
    public class SEListViewLayoutManager
    {
        #region 常量
        ...
        private SEListView _imageListView;
        ...
        private SEListViewRenderer _renderer;
        protected SEListViewRenderer Renderer
        {
        ...
        public int Margin
        {
            ...
            {
                _margin = value;
                _itemSizeWithMargin = _itemSize + value;
            }
        }

        private int _itemSizeWithMargin;
        internal int ItemSizeWithMargin
        {
        ...
        internal void OnItemsRemoved(List<SEListViewItem> items)
        {
        ...
        /// <returns></returns>
        public ListViewItemVisibility IsItemVisible(SEListViewItem item)
        {
            int itemIndex = _imageListView.Items.IndexOf(item);

            if (_imageListView.Items.Count == 0)
                return ListViewItemVisibility.NotVisible;
            ...
            if (itemIndex < _firstPartiallyVisible || itemIndex > _lastPartiallyVisible)
                ...
                return ListViewItemVisibility.Visible;
            else
                return ListViewItemVisibility.PartiallyVisible;
        }
        ...
        public Rectangle GetItemBounds(int index)
        {
            ...
            location.Y += index * _itemSizeWithMargin;

            location.X = ItemsArea.X;

            return new Rectangle(location, new Size(ItemsArea.Width,_itemSize));
        }

        public List<SEListViewItem> GetItems()
        {
        ...
        /// </summary>
        /// <returns></returns>
        public List<SEListViewItem> GetVisibleItems()
        {
        ...
        private void CalculateGrid()
        {
            ...
            _rowCount = (int)System.Math.Floor((float)itemArea.Height / (float)_itemSizeWithMargin);
        ...
        private void CalculateVisibleItemsRange()
        {
            ...
            _firstPartiallyVisible = (int)System.Math.Floor((float)offSet / (float)_itemSizeWithMargin);
            _lastPartiallyVisible = (int)System.Math.Ceiling((float)(offSet + itemAreaHeight) / (float)_itemSizeWithMargin) - 1;
            ...
            _firstVisible = (int)System.Math.Ceiling((float)offSet / (float)_itemSizeWithMargin);
            _lastVisible = (int)System.Math.Floor((float)(offSet + itemAreaHeight) / (float)_itemSizeWithMargin) - 1;
        ...
        private void UpdateScrollBars()
        {
            if (_imageListView.Items.Count > 0)
            {
                ...
                _vScrollBar.Maximum = Math.Max(0,
                    (int)Math.Ceiling(
                    (float)_imageListView.Items.Count) * _itemSizeWithMargin - 1);
                _vScrollBar.LargeChange = ItemsArea.Height;
                _vScrollBar.SmallChange = _itemSizeWithMargin;
        ...
        private void SelectItemsByRectangle(Rectangle rect)
        {
            ...
            int startRow = (int)Math.Floor((float)(Math.Min(pt1.Y, pt2.Y) + viewOffset - ItemsArea.Y) /
                  (float)this._itemSizeWithMargin);
            ...
            if (ShiftKey == false)
            {
                List<SEListViewItem> currentSlectedItems = _imageListView.GetSelectedItems();
            ...
            SEListViewItemCollection allItems = _imageListView.Items;
            //使框选区内的项都处于选中状态
