...
namespace ListViewTest
{
    ...
    /// </summary>
    public class SEListViewRenderer
    {
        #region 受保护的成员
        ...
        internal SEListViewTheme Theme { get; set; }

        protected SEListViewLayoutManager _layoutManager;
        internal SEListViewLayoutManager LayoutManager { get { return _layoutManager; } }

        #region 构造

        public SEListViewRenderer(SEListViewLayoutManager layoutManager)
        {
        ...
        public void RenderItem(Graphics g, SEListViewItem item)
        {
            if (LayoutManager.Suspend)
                return;

            if (LayoutManager.IsItemVisible(item) == ListViewItemVisibility.NotVisible)
                return;
        ...
        internal virtual void OnItemsRemoved(List<SEListViewItem> items)
        {
        ...
        /// <param name="bounds"></param>
        /// <param name="item"></param>
        internal virtual void DrawItemContent(Graphics g, Rectangle bounds, SEListViewItem item)
        {
            //显示debug信息
        ...
        private void RenderItems(Graphics g)
        {
            ...
            List<SEListViewItem> items = LayoutManager.GetVisibleItems();
            foreach (SEListViewItem item in items)
            {
                RenderItem(g, item);
            }
        }
